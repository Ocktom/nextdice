...
func execute(action_source_cell: Cell, action_target_cell: Cell, context:= {}):
	...
	for x in hunted_enemies:
		...
		ActionManager.request_action("projectile_attack",{"projectile_name" : "purple_ball", "amount" : PlayerStats.player_int},action_source_cell,x.current_cell)
		...
		ActionManager.request_action("damage_unit",{"amount" : PlayerStats.player_int-1,"damage_name" : "magic"},action_source_cell,x.current_cell)
