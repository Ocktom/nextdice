...
func execute(action_source_cell: Cell, action_target_cell: Cell, context:= {}):
	...
	var hunted_enemies = Global.grid.get_enemies_with_status("frost")
	...
	for x in hunted_enemies:
		...
		await Global.action_manager.request_action("projectile_attack",{"projectile_name" : "purple_ball", "amount" : Global.player_stats.player_int},action_source_cell,x.current_cell)
		...
		await Global.action_manager.request_action("damage_unit",{"amount" : Global.player_stats.player_int-1,"damage_name" : "magic"},action_source_cell,x.current_cell)
